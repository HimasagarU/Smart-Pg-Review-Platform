    ...
    <div className="page-enter" style={{
      ...
      <div className="card" style={{ padding: "40px", width: "100%", maxWidth: "420px" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div>
            <label style={{ fontSize: "0.85rem", color: "var(--text-secondary)", fontWeight: 500, marginBottom: "6px", display: "block" }}>
              Email
            </label>
          ...
          {error && (
            <div style={{ padding: "10px 14px", background: "var(--danger-light)", borderRadius: "var(--radius-sm)", color: "var(--danger)", fontSize: "0.85rem" }}>
              {error}
            </div>
          )}
        ...
        <p style={{ textAlign: "center", marginTop: "20px", fontSize: "0.85rem", color: "var(--text-muted)" }}>
          ...
          <Link href="/auth/register" style={{ color: "var(--accent-primary)", textDecoration: "none", fontWeight: 600 }}>Register</Link>
        </p>
      </div>
    </div>
  );
